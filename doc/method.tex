...
In order to achieve this we define the probability of a fractional state, $X$, to be
\begin{equation}
P_f = \frac{\overline{P_i}\beta \pi_1(X)e^{-k\left\|X - \left[X\right]\right\|}}{\overline{\pi_1}}
\end{equation}
where $\overline{P_i}$ is the average probability of the integer states, $\beta=\frac{n_i}{n_f}$ is the ratio of integer states to fractional states $\pi_1(X)$ is the number of non-degenerate pivots from state $X$, $k$ is a tunable parameter, $\left[X\right]$ is the vector that results from rounding each element of $X$ to the nearest integer and $\left\| . \right\|$ is the L1-norm and $\overline{\pi_1}$ is the average number of non-degenerate pivots in fractional states. 
...
\]
This can be calculated without doing a pivot by just calculating $B$ so we only need to pivot if the proposal is accepted, making transitions between fractional states much more efficient than between integer states.

The proportion of samples that are null samples (i.e. samples coming from fractional states) is expected to be simply the normalised sum of probabilities of all fractional states. If we assume the correlation between the number of non-degenerate pivots and the fractionality is small then the normalised total fractional probability is approximated by
\[
P_F \approx \frac{\overline{ e^{-k\left\|X - \left[X\right]\right\|}}}{1 + \overline{e^{-k\left\|X - \left[X\right]\right\|}}}
\]
where the overbar on the exponential denotes the average over all fractional states.

Since we don't know $\overline{P}$, $\beta$, $\overline{\pi_1}$ or $\overline{e^{-k\left\|X - \left[X\right]\right\|}}$ during the sampling process, in practice we can approximate $\overline{\pi_1}$ with its expectation value calculated from the samples so far.

The sum of probabilities of Integer/Fractional points can is given by the expectation value of $\frac{1}{P(X)}$ which can be estimated from the samples from each class of vertex.

 Also from the samples so far we can build an approximation of the distribution of $\left\|X - \left[X\right]\right\|e^{-k\left\|X - \left[X\right]\right\|}$ by kernel density approximation from the samples from fractional states. If we divide this by $e^{-k\left\|X - \left[X\right]\right\|}$ we get the probability distribution over $\left\|X - \left[X\right]\right\|$ which allows us to calculate $P_F$ and to set $k$ to ensure that $P_F$ is of a desirable value (say 0.01). We can then compare this against the proportion of integer to fractional samples and adjust our estimate of $\overline{P_i}\beta$ accordingly. Initially we set $\beta = 1$ and approximate $\overline{P_i}\beta$ with the square root of the expectation value of $P$ calculated from the samples so far (since the expectation of $P$ is the mean square of $P$).

approximate them by taking the averages over the states we have visited so far. Since we start on a valid sample, this is always well defined.
