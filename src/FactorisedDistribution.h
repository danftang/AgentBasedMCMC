...
//public:
//
//    template<typename AGENT, typename= std::is_same<ModelState<AGENT>,DOMAIN>>
//    static thread_local ModelState<AGENT> modelState;
//
...
//
//    template<typename AGENT, typename = std::is_same<ModelState<AGENT>, DOMAIN>>
//    std::vector<SparseFunction<std::pair<double, bool>, const Trajectory<AGENT> &>> trajectoryLogFactors(int time) {
...
//                        for (int agentId: modelFactor.dependencies) {
//                            modelState<AGENT>[agentId] = trajectory[State<AGENT>(time,agentId)];
//                        }
//                        return modelFactor(FactorisedDistribution<DOMAIN>::modelState<AGENT>);
//                    },
