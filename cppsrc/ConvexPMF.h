...
class ConvexPMF: public PMF {
    ...
    double logProb(const std::vector<double> &X) { return convexSupport.isValidSolution(X)?logPrior(X):0.0; }
    ...
    ConvexPMF &operator *=(const ConvexPMF &other) {
        ...
        for(int c=1; c<=other.convexSupport.nConstraints(); ++c) {
            convexSupport.addConstraint(other.convexSupport.getConstraint(c));
        }
        return *this;
    }
