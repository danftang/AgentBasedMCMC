...
class PivotBetweenExpts {
    ...
    fun pivotBetweenSolutions() {
        ...
        simplex.MPsolve(DoubleArray(abmMatrix.nCols) { Random.nextDouble() }.asList())
        val Xa = HashIntVector(simplex.X)
        simplex.MPsolve(DoubleArray(abmMatrix.nCols) { Random.nextDouble() }.asList())
        val Xb = HashIntVector(simplex.X)
        assert(simplex.M * Xa == simplex.B)
        assert(simplex.M * Xb == simplex.B)
        println(Xa)
        println(Xb)

        // setup initial state at first solution
        simplex.pivotToSolution(Xa)
        simplex.setPivotedSolutionValues(true)
        println(simplex.X - Xa)
        assert(simplex.X == Xa)

        // now make path to other solution
        while(simplex.pivotTowardsRandom(Xb)) {
            assert(simplex.M * Xa == simplex.B)
            assert(simplex.M * Xb == simplex.B)
            simplex.setPivotedSolutionValues(true)
...
//                    ?:throw(RuntimeException("Found loop with no valid pivot point ${M.columns[j]}"))
    ...
    fun Simplex.pivotTowardsRandom(Xb: HashIntVector): Boolean {
        val alreadyPivotedIn = pivotPoints.toSet()
        val colsToBePivotedIn = Xb.keys.filter { !alreadyPivotedIn.contains(it) }
        ...
        if(colsToBePivotedIn.size == 2) {
            colsToBePivotedIn.forEach { j ->
                println()
//                println(M.columns[it].sortedBy { it.key })
//                if(M.columns[it].all { (i, Mij) -> B[i] - Mij >= 0 }) println("Can semi-pivot up")
//                if(M.columns[it].all { (i, Mij) -> B[i] + Mij >= 0 }) println("Can semi-pivot down")
                val XaRows = M.columns[j].keys.filter { i -> B[i] != 0 }.toSet()
                val XbRows = M.columns[j].keys.filter { i -> Xb.keys.contains(pivotPoints[i]) }.toSet()

                println("Xa and Xb = ${ XaRows.intersect(XbRows).associateWith { i -> "${M[i,j]}:${B[i]}" }.entries.sortedBy { it.key } }")
                println("Xa not Xb = ${ XaRows.minus(XbRows).associateWith { i -> "${M[i,j]}:${B[i]}" }.entries.sortedBy { it.key } }")
            ...
            val intersection = M.columns[colsToBePivotedIn[0]].keys.intersect(M.columns[colsToBePivotedIn[1]].keys)
            println("Loop intersection ${
                intersection
                    .associateWith { Triple(M[it, colsToBePivotedIn[0]], M[it, colsToBePivotedIn[1]], B[it]) }
                    .entries
                    .sortedBy { it.key }
            }")
        }
...
//        }
    ...
    fun Simplex.pivotTowardsLoop(Xb: IntVector, col: Int): Boolean {
        return true
    }
